<template>
  <v-navigation-drawer id="app-drawer" v-model="inputValue" app dark fixed floating persistent mobile-break-point="991" width="260" style="margin-top:74px;">
    <v-img :src="image" :gradient="sidebarOverlayGradiant" height="100%">
      <v-layout class="fill-height" tag="v-list" column>
        ...
        <v-list-tile v-for="(link, i) in links" :key="i" :to="link.path" :active-class="color" avatar class="v-list-item">
          <v-list-tile-action>
  ...
  export default {
    ...
    computed: {
      ...
      links: {
        get() {
          var index = 0;
          menu.toolBarMenu.forEach(item => {
            console.log("item.path",item.redirect)
             console.log("this.$route.path",this.$route.path)
            if(item.redirect === this.$route.path) {
                index = item.drawerMenuIndex;
            }
          });
          return menu.drawerMenu[index];
        },
