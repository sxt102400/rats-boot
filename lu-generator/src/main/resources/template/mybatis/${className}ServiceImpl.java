...
public class ${className}ServiceImpl implements ${className}Service {
     ...
     */
    @Override
    public long count(${className} condition ) {
...
<#if table.pkCount == 1>
     ...
     */
    @Override
    public Page<${className}> selectPage(${className} condition, Pageable pageable) {
        Example example = buildExample(condition);
        RowBounds rowBounds = new RowBounds( (int)(pageable.getOffset()), (int)(pageable.getLimit()));
        long totalCount = ${className?uncap_first}Mapper.countByExample(example);
        List<${className}> data = ${className?uncap_first}Mapper.selectByExample(example, rowBounds);
        Page<${className}> page = new PageImpl(data,pageable,totalCount);
        return page;
    }
...
<#if table.pkCount == 1>
    ...
    public Example buildExample(${className} condition){
